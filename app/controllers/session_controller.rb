class SessionController < ApplicationController
  ...
  def login
    ...
    if command.success?
      ...
      cookies.signed[:jwt] = {value: command.result[:token], httponly: true, same_site: :lax, expires: 1.hour.from_now}
  ...
  def signup
    ...
    if user.save
      ...
      cookies.signed[:jwt] = {value: jwt, httponly: true, same_site: :lax, expires: 1.hour.from_now}
      render json: UserSerializer.new(user).to_serialized_json
    else
